...
The variables that have errors are position and block.
...
This code is counting the number of responses for these variables. There is only 199 rows with one response. This means that there is one row with more than one response which is messing up the data. 
...
The observation where week=2, lure=Control, block=D, and position=2 is in the data set and should not be.  

The observation where week=2, lure=Control, block=E, and position=6 is missing from the data set.
...
wasps %>%
  ...
  filter(week==2, lure=='Control', block=='E')
```
