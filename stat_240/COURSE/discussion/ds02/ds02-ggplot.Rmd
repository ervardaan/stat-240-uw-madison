...
## byron vardaan kapoor and 2 other people
...
ggplot(data=planets,mapping=aes(x=radius,y=mass)) +geom_point()+scale_y_log10() + scale_x_log10() +
...
```{r question-2}
## COMPLETE CODE TO MAKE THIS PLOT
ggplot(data=planets,mapping=aes(x=radius,y=mass)) +geom_point(aes(color=method)) +
...
```{r}
## COMPLETE CODE TO MAKE THIS PLOT
ggplot(data=planets) + geom_bar(mapping=aes(x=method)) +
...
1)Transit Transit
2)They both are diredtly proportional to each other
3)It is approximately 30 times the radius of Earth
...
```{r}
## COMPLETE CODE TO MAKE THIS PLOT
ggplot(data=planets,mapping=aes(x=log10(mass))) + geom_histogram(color="red",fill="blue") +
