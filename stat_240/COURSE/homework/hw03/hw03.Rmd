...
planets %>%  
  ...
  arrange(mass)
##radius1=planets %>% filter(planet=="Earth") %>% select(radius)
##planets %>% filter(mass<mass1) %>% select(-id) %>% arrange(mass)
```
...
```{r}
### Add your code here
question_2=planets %>% filter(!is.na(year) & !is.na(mass) & method=="Radial Velocity" ) %>% group_by(year) %>% mutate(planets_given=n(),mass_min=min(mass)) 
question_2 %>% print(n=10,c=everything())
#question_2
ggplot(data=question_2)+geom_point(mapping=aes(x=year,y=mass_min,size=planets_given)) +scale_y_log10()+xlab("annual change")+ylab("Minimum mass calculated") + ggtitle("distirbution of mass of planets with respect to year of discovery")

```

**Describe the pattern between year and minimum mass of planet discovered using Radial Velocity.**

> Over the years,the mass of discovered planets decreased and the no of planets which were discovered over a particular year increased
...
### Add your code here
 ...
 planets %>% count(method)
planets_int = planets %>% group_by(year) %>% summarize(total_count = n())
planets1 = planets %>% filter( method == "Microlensing"  | method == "Transit" | method == "Radial Velocity" | method == "Imaging" )
#planets1
planets1=planets1 %>% group_by(method, year)
planets1 = planets1 %>% right_join(planets_int) %>% arrange(year, method)
planets2 <- planets1 %>% select(year, total_count, method) %>% group_by(method, year) %>% summarize(n = n()) %>% arrange(year) %>% right_join(planets_int)
planets2 <- planets2 %>% mutate(prop = n / total_count) %>% select(-total_count) %>% drop_na()
planets2
...
```{r}
### Add your code here
planets2 %>% print(n=10,c(everything))
...
```{r}
### Add your code here
ggplot(data=planets2)+geom_col(mapping=aes(x=year,y=prop,fill=method)) +xlab("year change")+ylab("proportion differences") +ggtitle("chnages in proportion seen over a course of many years")
```
...
```{r}
## Add your code here
dataset=planets2 %>% filter(year>=2010 & year<=2022 & (method!="Transit" & method!="Radial Velocity") )
dataset
ggplot(data=planets2)+geom_col(mapping=aes(x=year,y=n,fill=method),position=position_dodge2(preserve="single"))+scale_x_continuous(breaks=c(2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022),labels=c("2010","2011","2012","2013","2014","2015","2016","2017","2018","2019","2020","2021","2022"))+xlab("planets")+ggtitle("planets by year and method")
```
...
```{r}
## Add your code here
h1=official %>% arrange(desc(tmax)) %>% distinct(tmax) %>% head(5)
h1
h2=official %>% group_by(tmax) %>% summarize(tmax,date) %>% arrange(desc(tmax)) %>% filter(tmax%in%c(h1[["tmax"]]))
h2
```
...
## Add your code here
#official 
day1=official %>% mutate(our_day=day(date),our_month=month(date)) %>% filter(prcp>0) %>% group_by(our_month) %>% mutate(monthN=n()) %>% group_by(our_day,our_month) %>% summarize(prop=n()/monthN) %>% distinct(prop)
day1
```
...
```{r}
## Add your code here
nm=official %>% mutate(y=year(date),m=month(date)) %>% filter(y>1990 & y<2021) %>% group_by(m) %>% summarize(mean_c=mean(tavg))
a22=official %>% mutate(y=year(date),m=month(date)) %>% filter(y==2022) %>% group_by(m) %>% summarize(av=mean(tavg))
test=unlist(c(merge(nm,a22) %>% mutate(difference=av-mean_c) %>% filter(difference>0) %>% select(m)))
test2=month.abb[test]
test2
```
...
```{r}
## Add your code here
test10=official %>% mutate(y=year(date)) %>% group_by(y) arrange(desc(tavg)) %>% head(10)
tester=official %>% mutate(y=year(date)) %>% filter(tavg %in% c(test10[["tavg"]])) %>% filter(y>=2000)
tester
```
...
```{r}
meanT=official %>% mutate(y=year(date),m=month(date),d=day(date)) %>% filter(m==1) %>% group_by(d)  %>% summarize(dm=mean(tavg)) %>% arrange(desc(dm)) %>% head(25)
askedQ=mean(meanT[["dm"]])
askedQ
```

> The temperature(mean daily) in jan 2023 was about 8 degrees more than any other previous year
...
```{r}
jT=official %>% mutate(y=year(date),m=month(date),day=day(date)) %>% filter(m==1) %>% group_by(y) %>% summarize(avg=mean(tavg))
ggplot(jT,aes(x=y,y=avg)) +geom_smooth() +geom_hline(yintercept = 27.01,color="red")+xlab("year change")+ylab("month's mean temp")+ggtitle("variation in temperature over different years")
```
