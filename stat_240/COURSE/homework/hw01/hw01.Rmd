...
```{r}
#get all numbers first inside  a vector-then find the sum of all the #values
collect=seq(from=2,by=2,to=500)
sum1=sum(collect)
sum1
```
...
2^seq(0, 5, 1)
```
>the symbol is used to raise 2 to the power of the individual elements of the vector-first a sequence of developed by the function and then 2 is raised to power of those elements
...
```{r}
min1=min(rnorm(10,mean=20,sd=5))
min1
#should have written min(x)
max1=max(rnorm(10,mean=20,sd=5))
max1
#should have written max(x)
...
```{r}
median1=median(10,mean=20,sd=1)
#should have written median(x)
median1
```

>the median occurs in the middle of a list-in a list of even numbers there is no middle number and so we take the average of the 2 middle numbers
...
```{r mychunk }
x=seq(from=4,to=44,by=5)
median1=median(x)
sum1=sum((x-median1)^2)
#should have written mean1=mean(x)
#should have written sum1=(sum(x=mean1)^2)
sum1
```
