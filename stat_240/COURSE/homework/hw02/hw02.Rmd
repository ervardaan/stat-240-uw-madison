...
ggplot(monona, aes(x = duration)) +
  ...
  xlab("Days Covered with ice")+
  ylab("Total")+
  ggtitle("changes in no of days covered by ice wrt time")
```
...
the smooth graph points the data points but in for of a line with some confidence level-we have removed the confidence level around the line by using "se=FALSE". 

- Describe the pattern of the curve.

### Response
This line shows us the trend as to how does the duration change when year1 changes-it shows that the two variables are inversely proportional
...
ggplot(monona, aes(y=duration))+
  ...
  ylab("no of days frozen")+
  ggtitle("analysis of lake being frozen-distribution of frozen days")
```
...
```{r}
ggplot(data=monona,mapping=aes(x=winter))+
  geom_bar()+xlab("durarion intervals")+
  ylab("count of no of intervals'values")+
  facet_wrap(~ff_cat)+
  ggtitle("analysis and relation of duration with half month period intervals",subtitle="duration's translation to find number of intervals which had first breeze in half month period")
```
...
We didn't count the counter of duration's values by ourselves-we gave this task to R
If we want to give one variable or counter to y axis then we can use geom_col() option
...
ggplot(monona, aes(x = ff_x, y = duration, color = period50)) +
  ...
  ggtitle("duration and its relation with first freeze date")+
  xlab("numerical coding of the first freeze")+
  ylab("50 period of time")+
...
answer:If it were 1855-1900s, then we would expect duration to be ~88 days
if it were 1901-1950s, then we would expect ~85 days
if it were 1951-2000s, then we would expect ~83 days
if it were 2000s-2022, then we would expect ~81 days
...
```{r}
ggplot(data=monona,mapping=aes(x=ff_x,y=duration))+geom_point()+facet_wrap(~period50);
```
...
Yes,coloring and faceting helps us focus on a particular graph and see the trends between every period of time.
