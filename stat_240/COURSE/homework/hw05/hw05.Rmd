...
> `num` is .number of rows in which pct_f_bach is more than pct_m_bach..

> `n` is no of data enteries in all the rows...

> `p_1` is mean of the rows's values where pct_f_bach is more than pct_m_bach...

> `p_2` is mean of the values of rows for which both the columns are equal..
...
```{r}
ggplot(data=education_orig,mapping=aes(x=pct_f_bach,y=pct_m_bach))+geom_point()+geom_smooth(color="blue")+geom_abline(slope=1,intercept=0)+ggtitle("comparison of male and female data")+xlab("no of females") +ylab("no of males")
```
...
```{r}
education=education_orig %>% drop_na() %>% rename("female"="pct_f_bach","male"="pct_m_bach") %>%  mutate(zip=as.character(zip)) %>% pivot_longer(cols=contains("male"),names_to = "sex",values_to = "pct_bachelors") 
education
```
...
```{r}
education=education %>% left_join(adult_pop_sum)
asked_data=education %>% group_by(zip) %>% filter(pct_bachelors==0 | pct_bachelors==100) %>%  arrange(desc(total_pop))
asked_data
education
```
...
```{r}
#education
#education %>%  filter(is.na(pct_bachelors))
#new_data=education %>% filter( !is.na(pct_bachelors))

#new_data
#education1<-education %>%  group_by(zip,sex) %>% mutate(total_population=pct_bachelors * total_pop/100) %>%  ungroup() %>%  group_by(sex) %>% summarize(sum_final=sum(total_population)) 
#education1=education %>% ungroup() %>% group_by(sex) %>%  mutate(number=sum(total_pop))
#%>%  group_by(sex) %>%  summarize(sum=sum(number))
#education1


education  %>%  drop_na() %>% group_by(zip,sex) %>%  mutate(number=sum((pct_bachelors*total_pop)/100)) %>%  group_by(sex) %>% summarize(total_by_sex=sum(number)/sum(total_pop)*100)
```

> The percentages of women and men ages 25 and older in Wisconsin with a bachelors degree are ...
...
education
adult_obesity_sum
collected_data=adult_obesity_sum %>%  full_join(education,by=c("zip","sex")) %>%  group_by(zip,sex) %>% mutate(pct_obese=(sum1/total_pop.x)*100)
ggplot(collected_data,aes(y=pct_obese,x=pct_bachelors,color=sex)) +geom_point() +geom_smooth(aes(color=sex))
```
