...
> We know in a simple regression problem, the best fit line would pass through the mean values of x and y, i.e. through x_bar and y_bar. And the predicted value of y for a given x lies on the best fit line. So with x = 20, y would be on the best fit line and we know that the corresponding y for x_bar is 100 and both x and x_bar is 20, so y_hat is 100 for x=20.
...
> The possible values are 70, 100 and 120. This is because, the slope is correlation(r)*(standard deviation of y/ standard deviation of x). That is equal to r*(15/5).
So slope is 3r.
Also we know that the line passes through the point (20, 100) since they are the means of x and y.
We can find the value of intercept from this with relation to r. We know, y_bar = intercept + slope * x_bar. So, intercept = y_bar - slope * x_bar, thus intercept is 100- 60r.
The equation of the regresion line is y_hat = intercept + slope*x, so y_hat = 100-60r+3r*30 (Since x=30), y_hat = 100 + 30r. 
By definition we know that the value of r is betwen -1 and 1. So, the value of y_hat is between 100-30=70 and 100+30=130. The only option in that range is 70, 100 and 120.
...
> As we observe, we get the same values.
...
> We can observe that there are a group of residuals that are below for a certain range of x or Length and for other ranges of x or Length the residuals are above the y = 0 line, indicating that they are categorically overfitted or underfitted and deviate from the assumption that the mean of the distributions of residuals is 0. This indicates that the simple linear model does not account for some characterisitc of the data and is not the best model to model the data.
...
> This is the Residual standard error on 25 degrees of freedom appearing in the fitted regression. object summary.
...
> As we observe that the mean of the residuals is very close to zero.
...
ggplot(marathon, aes(x=Age, y=Time)) + 
  ...
  xlab("Age of women in marathon in 2010") + 
  ylab("Time taken by them in 2010") + 
  ggtitle("Time vs Age for female runners in 2010")
```
...
ggplot(marathon, aes(x=Age, y=residual)) + 
  ...
  geom_smooth() +
  geom_hline(yintercept = 0) +
  xlab("Age of female runner in 2010") + 
  ylab("Residuals")  +
  ggtitle("Residual vs Age of runner")
...
> There is a strong evidence of non-linearity since we observe a parabolic distribution for the residuals as evident from the graph. The density of positive redidual values are much more resulting in a bigger right tail in the density curve, that indicates a non-linear relationship.

- Is there evidence that the standard deviation of the residuals varies substantially with changes in age?

> No, there is no evidence that the standard deviation of the residual varies substantially with changes in Age since the  data does not seem to have an overall shape or heavier on either side of the line at y = 0. It seems to be randomely distributed around the line indicating a mean of 0 for the distribution.


- Is there evidence that the error distribution for individual residuals is not symmetric?

> There is evidence that the data is not symmetric since from the density plot of residuals, we observe that the data is centered around 0 indicating a mean of 0 but, however one tail (right) is significantly longer than the other tail (left)
