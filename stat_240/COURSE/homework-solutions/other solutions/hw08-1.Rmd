...
gnorm(200, 40) +
  geom_norm_fill(200, 40, mode1 = qnorm(0.9, 200, 40), mode2 = qnorm(0.1, 200, 40))
a= qnorm(0.9, 200, 40)
b = qnorm(0.1, 200, 40)
x
y
...
> x corresponds with mean and sd of the normal distribution
but z corresponds to the relative position of data values wrt the mean and sd-as p never changes and so the values of z also don't change
...
#coloring the portions wrt the values given by a and b
```
...
```{r}
pnorm(2*39.37, 70, 3, lower.tail = FALSE)#have to include the tail attribute
qnorm(0.95, 70, 3)
```
...
#adding the variables to make a dataframe
print(df2)
```
...
p6 = prob5_df %>%
  filter(x>=20)
pl6 = ggplot(p6, aes(x = x, y = prob)) +
  geom_col(width = 1, fill = "lightgray", color = "black",) +
  geom_col(width = 1 ,fill = "firebrick", color = "black",
           data = prob5_df %>% filter(x >= qbinom(0.90, n, p))) +
...
> a_2 is closer to the actual calculqtion because the variables/columns used to calculate the binomial have width of 1 unit(s).
...
no-beacuse the probability of 35 coins' heads is very low and is unlikely to occur-around 0.01% and so is highly improbable
...
prob7_df = tibble(
  u_bar = map_dbl(1:B, ~mean(runif(n, aa, bb))))
```
...
mean of 100+sd of 5(rounded values)
...
graph10 = ggplot(prob7_df, aes(x = u_bar)) +
  geom_density(color = "blue") + 
  geom_norm_density(100, 5, color = "red")
...
> it is not equal to the simulation because the model is not scalable to the real world dataset as the model works for the smaller aount of values while here we are getting large number of values
...
> a is around 2.6
area under the graph is more than true probability because it is very far from the true probability and has the highest value of sd
